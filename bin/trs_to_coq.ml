...
module T = struct
  ...
  let rec iter_idents t ~f =
    match t with
    | Var s -> f s
    | App (fname, args) -> f fname; List.iter ~f:(fun t -> iter_idents t ~f) args

  let idents t =
    let names = ref [] in
    iter_idents t ~f:(fun s -> if List.mem ~equal:String.equal !names s then () else names := s :: !names);
    !names
  ...
  let find_argnum t fname =
    let rec loop t =
      ...
      | App (f, args) ->
        if String.(f = fname) then
          Some (List.length args)
        else
          List.find_map ~f:loop args in
    loop t
  ...
  let rec to_coq t =
    ...
    | Var s -> s
    | App (f, args) ->
      let args_str = List.map ~f:(
        fun c -> match c with Var _ -> to_coq c | App _ -> "(" ^ to_coq c ^ ")" ) args |> String.concat ~sep:" " in
...
module E = struct
  ...
  let to_coq t vars =
    let idents = ref [] in
    iter_vars t ~f:(fun s -> if List.mem ~equal:String.equal !idents s then () else idents := s :: !idents);
    let prods = List.filter ~f:(fun s -> (List.mem ~equal:String.equal vars s)) !idents in
    let prods = List.rev prods in
    String.concat ~sep:""
    ((if List.length prods > 0 then ["forall ";String.concat ~sep:" " prods; ", ";] else []) @ [T.to_coq (fst t); " = "; T.to_coq (snd t); ""])
end
...
module Trs = struct
    ...
    | hd :: rest ->
      if String.is_prefix hd ~prefix:"(RULES" then
        let rec loop acc rest =
          ...
          | ")" :: t -> (acc, t)
          | h :: t -> loop (h :: acc) t
          | [] -> failwith "RULES: parse error: []" in
        let rules, rest = loop [] rest in
        let rules = List.filter ~f:(fun s -> String.length (String.strip s) > 0) rules in
        (List.map ~f:E.of_string rules, rest)
      else
        failwith "RULES: parse error: not (RULES";
    | _ ->
      failwith "RULES: parse error: not (RULES"
  ...
  let parse lines =
    let rec loop acc lines =
      ...
      | hd :: tl ->
        ...
        | _ -> Printf.eprintf "ignoring line: %s\n" hd; loop acc tl) in
  ...
  let to_coq t =
    ...
    let argnum_of_funname fname = List.find_map ~f:(fun e -> E.find_argnum e fname) t.rules  |> Option.value_exn in
    let functions =
      funnames
      |> List.map ~f:(fun s ->
    ...
    let axioms =
      ...
      |> String.concat ~sep:"\n" in
    let axiom_names = List.mapi ~f:(fun i _ -> "ax" ^ (string_of_int i)) t.rules in
    String.concat ~sep:"\n"
    [ {|
...
Variable G : Set.
    ...
    ; (String.concat ~sep:""
      ["Complete ";String.concat ~sep:" " axiom_names;" : ";String.concat ~sep:" " funnames;" : hint_compl."]
    )
