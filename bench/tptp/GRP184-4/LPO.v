...
(* axioms *)
Parameter G : Set.
Axiom p21x_5 : forall X Y : G, (inverse (least_upper_bound X Y)) = (greatest_lower_bound (inverse X) (inverse Y)).
Axiom p21x_4 : forall X Y : G, (inverse (greatest_lower_bound X Y)) = (least_upper_bound (inverse X) (inverse Y)).
Axiom p21x_3 : forall X Y : G, (inverse (multiply X Y)) = (multiply (inverse Y) (inverse X)).
Axiom p21x_2 : forall X : G, (inverse (inverse X)) = X.
Axiom p21x_1 : (inverse identity) = identity.
...
Complete p21x_5 p21x_4 p21x_3 p21x_2 p21x_1 monotony_glb2 monotony_lub2 monotony_glb1 monotony_lub1 glb_absorbtion lub_absorbtion idempotence_of_gld idempotence_of_lub associativity_of_lub associativity_of_glb symmetry_of_lub symmetry_of_glb associativity left_inverse left_identity :  : hint
  for ((multiply (least_upper_bound a identity) (inverse (greatest_lower_bound a identity))) = (multiply (inverse (greatest_lower_bound a identity)) (least_upper_bound a identity))).

(* Goal *)
Theorem check : (multiply (least_upper_bound a identity) (inverse (greatest_lower_bound a identity))) = (multiply (inverse (greatest_lower_bound a identity)) (least_upper_bound a identity)).
Proof.
