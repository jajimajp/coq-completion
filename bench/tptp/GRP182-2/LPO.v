...
(* axioms *)
Parameter G : Set.
Axiom p17a_3 : forall X Y : G, (inverse (multiply X Y)) = (multiply (inverse Y) (inverse X)).
Axiom p17a_2 : forall X : G, (inverse (inverse X)) = X.
Axiom p17a_1 : (inverse identity) = identity.
...
Complete p17a_3 p17a_2 p17a_1 monotony_glb2 monotony_lub2 monotony_glb1 monotony_lub1 glb_absorbtion lub_absorbtion idempotence_of_gld idempotence_of_lub associativity_of_lub associativity_of_glb symmetry_of_lub symmetry_of_glb associativity left_inverse left_identity :  : hint
  for ((least_upper_bound identity (greatest_lower_bound a identity)) = identity).

(* Goal *)
Theorem check : (least_upper_bound identity (greatest_lower_bound a identity)) = identity.
Proof.
