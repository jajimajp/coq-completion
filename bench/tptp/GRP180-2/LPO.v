...
(* axioms *)
Parameter G : Set.
Axiom p11_3 : forall X Y : G, (inverse (multiply X Y)) = (multiply (inverse Y) (inverse X)).
Axiom p11_2 : forall X : G, (inverse (inverse X)) = X.
Axiom p11_1 : (inverse identity) = identity.
...
Complete p11_3 p11_2 p11_1 monotony_glb2 monotony_lub2 monotony_glb1 monotony_lub1 glb_absorbtion lub_absorbtion idempotence_of_gld idempotence_of_lub associativity_of_lub associativity_of_glb symmetry_of_lub symmetry_of_glb associativity left_inverse left_identity :  : hint
  for ((multiply a (multiply (inverse (greatest_lower_bound a b)) b)) = (least_upper_bound a b)).

(* Goal *)
Theorem check : (multiply a (multiply (inverse (greatest_lower_bound a b)) b)) = (least_upper_bound a b).
Proof.
