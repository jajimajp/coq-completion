...
(* axioms *)
Parameter G : Set.
Axiom p23x_3 : forall X Y : G, (inverse (multiply X Y)) = (multiply (inverse Y) (inverse X)).
Axiom p23x_2 : forall X : G, (inverse (inverse X)) = X.
Axiom p23x_1 : (inverse identity) = identity.
...
Complete p23x_3 p23x_2 p23x_1 monotony_glb2 monotony_lub2 monotony_glb1 monotony_lub1 glb_absorbtion lub_absorbtion idempotence_of_gld idempotence_of_lub associativity_of_lub associativity_of_glb symmetry_of_lub symmetry_of_glb associativity left_inverse left_identity :  : hint
  for ((least_upper_bound (multiply a b) identity) = (multiply a (least_upper_bound (inverse a) b))).

(* Goal *)
Theorem check : (least_upper_bound (multiply a b) identity) = (multiply a (least_upper_bound (inverse a) b)).
Proof.
