...
module CompletionInput = struct
  type t = {
    ...
    ops : (string * int) list;
        (* function operators (name, arity), arity=0 if constant *)
  ...
  let of_fixpoint_constr env sigma (acc : t) (t : Constr.t) : t =
    if not (Constr.isFix t) then failwith "Not supported form of constant"
    else
      let _, (names, types, constrs) = Constr.destFix t in

      if
        Array.length names > 1
        || Array.length types > 1
        || Array.length constrs > 1
      then
        ...
        let rec pick_variables acc (constr : Constr.t) =
          match Constr.kind constr with
          | Lambda (name, types, constr) ->
              pick_variables (Context.binder_name name :: acc) constr
          | e -> (acc, e)
        in
        let vars, e = pick_variables [] constr in

        if not (Constr.isCase (Constr.of_kind e)) then
          failwith "Not supported form: match is not used toplevel in Fixpoint"
        else
          let ci, univs, _, (nr, _), _, c, arr =
            Constr.destCase (Constr.of_kind e)
          in

          (* c is in [match c with ...]*)
          if not (Constr.isRel c) then
            failwith
              "Not supported form: In [match c with ...], c should be a \
               variable"
          else
            let c = Constr.destRel c in
            Printf.printf "c: %d\n" c;
            let matched_pos = List.length vars - c in
            ...
            let cases =
              List.map
                (fun case -> (prefix ^ "." ^ List.hd case) :: List.tl case)
                cases
            in
            ...
            let localenv =
              List.map
                (fun name -> Pp.string_of_ppcmds (Names.Name.print name))
                vars
            in
            let name_str = name |> Names.Name.print |> Pp.string_of_ppcmds in
            let name_str = prefix ^ "." ^ name_str in
            let localenv = localenv @ [ name_str ] in
            (* To reconstruct from rel. Environ may be suitable. *)
            ...
            let gen_lhs (t : term) : term =
              ...
              let vars =
                List.map
                  (fun name ->
                    name |> Names.Name.print |> Pp.string_of_ppcmds |> mkVar)
                  (List.rev vars)
              in
              let args =
                List.mapi (fun i v -> if i = matched_pos then t else v) vars
              in
              mkTermApp f args
            in
            ...
            let rec aux (acc : t)
                (branches :
                  (Constr.t, Sorts.relevance) Constr.pcase_branch list)
                (cases : string list list) =
              match (branches, cases) with
              | [], [] -> acc
              | b :: bs, c :: cs ->
                  let f = List.hd c in
                  let lhs =
                    My_term.mkTermApp f (List.map mkVar (List.tl c)) |> gen_lhs
                  in
                  let localenv =
                    List.fold_left
                  ...
                  in
                  let _ = localenv in
                  let rec parse_term (t : Constr.t) : term =
                    match Constr.kind t with
                    | Rel i ->
                        mkVar (List.nth localenv (i - 1)) (* Rel is 1-indexed *)
                    | App (f, args) ->
                        let f =
                          match parse_term f with
                          | App (f, args) ->
                              failwith "Not supported form: nested App"
                          | Var s -> s
                        in
                    ...
                    | e ->
                        failwith
                          (Printf.sprintf "Not supported form: %s"
                             (label_of_constr e))
                  in
                  let _, rhs = b in
              ...
              | _ ->
                  failwith
                    "Not supported form: number of branches is not equal to \
                     number of cases"
            in
          ...
          if isConst sigma f then (
            ...
            let acc =
              ...
              | Def constr ->
                  ...
                  Feedback.msg_notice
                    Pp.(str "Def " ++ Printer.pr_constr_env env sigma constr);
                  acc
              | _ ->
                  failwith
                    "App: Function is not defined yet, which is not supported."
            in

            Feedback.msg_notice
              Pp.(
                Names.Constant.print constant
                ++ str ", args: len "
                ++ int (Array.length arg_arr));
            acc)
          else if isConstruct sigma f then (
            ...
            let acc =
              List.fold_left
                (fun acc l ->
                  ...
                  let f =
                    (Names.MutInd.to_string mutind
                    |> String.split_on_char '.' |> List.rev |> List.tl
            ...
            failwith
              (Printf.sprintf
                 "App: Function=%s is neather Const nor Construct, which is \
                  not supported."
                 (label_of_constr (EConstr.kind sigma f)))
      | Var id -> acc (* Nothing to do *)
      | t -> failwith ("Not implemented: " ^ label_of_constr t)
    in
    aux acc t
  ...
  let of_goal_concl env sigma (goal_concl : Evd.econstr) : t =
    ...
    let fail () =
      failwith
        "Current goal does not have form \"_ = _\", which is not currently \
         supported."
    in
    if not (isApp sigma goal_concl) then fail ()
    else
      (* App *)
      let f, arg_arr = destApp sigma goal_concl in
      (* This should be _ = _ *)
      if not (isInd sigma f) then fail ()
      else
        let (mutind, _), _ = destInd sigma f in
        if not ("Coq.Init.Logic.eq" = Names.MutInd.to_string mutind) then
          failwith
            "Current goal does not have form \"_ = _\" (\"=\" is \
             Coq.Init.Logic.eq), which is not currently supported."
        else if not (Array.length arg_arr = 3) then fail ()
        else
...
let rec eq_term_of_my_term (prefix : string option) (t : term) : Equation.Term.t
  ...
  | App (f, args) ->
      Equation.Term.mkApp (add_prefix f)
        (List.map (eq_term_of_my_term (Some prefix)) args)
...
let complete (axioms : Equation.t list) (constants : string list) hint_db_name =
  ...
  let constantsopt : constants option =
    Some (My_term.constants_of_list constants)
  in
...
let debug_add_single_axiom () =
  let eq =
    Equation.create ~varnames:[ "m" ]
      ~left:Equation.Term.(mkApp "plus" [ mkVar "O"; mkVar "m" ])
      ~right:Equation.Term.(mkVar "m")
  in
  ...
  let tactic =
    ...
    Tactics.intros <*> Tactics.simpl_in_concl <*> Tactics.reflexivity
  in
  let _, progress =
    Declare.Obls.add_definition ~pm:Declare.OblState.empty ~cinfo ~info ~uctx
      ~opaque:false ~tactic obl_info
  in
  let () =
    match progress with
    ...
    (function
      | Context.Named.Declaration.LocalDef _ ->
          Feedback.msg_warning
            (Pp.str "cople_auto: LocalDef is ignored (not implemented)");
          None
...
let get_constants_from_hyps sigma (hyps : EConstr.named_context) (set : string)
    : string list =
  List.filter_map
    (function
      | Context.Named.Declaration.LocalDef _ ->
      ...
      | Context.Named.Declaration.LocalAssum (annot, types) ->
          let name = Context.binder_name annot |> Names.Id.to_string in
          let is_constant =
            match EConstr.kind sigma types with
            | Constr.Ind (inductive, instance) ->
                set = Names.Label.to_string (Names.MutInd.label (fst inductive))
            | _ ->
                let warn =
                  Printf.sprintf "get_constants_from_hyps: ignore kind: %s\n"
                    (label_of_constr (EConstr.kind sigma types))
                in
                Feedback.msg_warning (Pp.str warn);
                false
          in
          if is_constant then Some name else None)
    hyps
...
let compl_auto (fixpoints : string list) =
  Proofview.Goal.enter (fun gl ->
      let concl, env, sigma, hyps =
        Proofview.Goal.(concl gl, env gl, sigma gl, hyps gl)
      in
      ...
      let input =
        List.fold_left
          (fun acc fixpoint ->
            let constr =
              const_body_of_constant_qualid (Libnames.qualid_of_string fixpoint)
            in
            CompletionInput.of_fixpoint_constr env sigma acc constr)
          input fixpoints
      in
      (* debug *)
      ...
      | Some s -> (
          ...
          let eqs =
            List.map
              (fun (left, right) ->
                ...
                Equation.create_with_constants ~constants ~left ~right)
              eqs
          in
          ...
          match
            List.find_opt
              (fun (_, s) ->
                match s with Tomaparser.Axiom -> false | _ -> true)
              proofs
          with
          ...
          | None ->
              ...
              let tactic prf =
                ...
                Printf.printf "t: %s\n" (My_term.to_trs t);
                let goal = My_term.to_constrexpr_raw t constants in
                let sigma, goal =
                  Constrintern.interp_constr_evars env sigma goal
                in
                let tcl1 = Tactics.simpl_in_concl <*> Tactics.reflexivity in
                let tcl2 = Auto.default_auto in
                Tactics.assert_by
                  (Names.Name.mk_name (Names.Id.of_string ("axiom" ^ fst rule)))
                  (* TODO: name *)
                  goal
                @@ Proofview.tclIFCATCH tcl2
                     (fun () ->
                       Printf.printf "Defined\n";
                       Proofview.tclUNIT ())
                     (fun (e, _) ->
                       Printf.printf "Err: %s\n" (Printexc.to_string e);
                       Feedback.msg_notice Pp.(str "ERRR");
                       tcl1)
              in

              let tcl =
                List.fold_left
                  (fun acc prf -> acc <*> tactic prf)
                  (Proofview.tclUNIT ()) (List.rev proofs)
              in
    ...
    let rewriteLR c at = cl_rewrite_clause c true (OnlyOccurrences [ at ]) None in *)
              ...
              let rewriteLR_with_binds c =
                ...
                general_rewrite ~where:None ~l2r:true (OnlyOccurrences [ 1 ])
                  ~freeze:true ~dep:true ~with_evars:false (c, binds)
              in
                ...
                let tcl =
                  List.fold_left
                    (fun acc (rule, _) ->
                      ...
                      let tac =
                        rewriteLR_with_binds
                          (EConstr.mkVar (Names.Id.of_string rewriter))
                      in
