...
module Term = struct
  ...
  let rec fold_vars f init = function
    ...
    | App (_, args) ->
      List.fold_left (fold_vars f) init args
    ...
    | App (f, args) ->
      let f : string =
        if isConst f then
          ...
          raise (Invalid_argument "not implemented function type") in
      let args : t list =
        List.map of_constr (Array.to_list args) in
      mkApp f args
    | Const (k, _) ->
...
let merge a b =
  let rec insert acc e = match acc with
    | [] -> [e]
    | (hd :: tl) as l -> if hd = e then l else hd :: insert tl e in
...
let varnames_in_term varnames term =
  let is_var name =
    List.exists (fun e -> e = name) varnames in
  let rec aux = function
    | Term.Var name ->
      if is_var name then [name] else []
    | App (_, args) ->
      List.fold_left merge [] (List.map aux args) in
  aux term

let varnames_in_left  t =
  varnames_in_term t.varnames t.left

let varnames_in_right t =
  varnames_in_term t.varnames t.right
...
let create ~varnames ~left ~right =
  { left; right; varnames }
...
let create_with_constants ~constants ~left ~right =
  ...
  let leftvars = Term.fold_vars (fun acc name ->
    if not (is_const name || is_in acc name) then name :: acc else acc) [] left in
  let varnames = Term.fold_vars (fun acc name ->
    if not (is_const name || is_in acc name) then name :: acc else acc) leftvars right in
  { left; right; varnames }
...
let rec find_eq_opt c =
  let f_is_eq_p f =
    if not (isInd f) then false
      ...
      "Coq.Init.Logic.eq" = Names.MutInd.to_string mutind in
  try
    match kind c with
...
let of_constr c =
  ...
  let varname_of_ind i =
    List.nth varnames (i - 1) in
  match find_eq_opt c with
  | Some c ->
...
let to_constr_expr t =
  let open Constrexpr in
  let ( <.> ) f g x = (f (g x)) in
  let vars = List.map (CAst.make <.> Names.Name.mk_name <.> Names.Id.of_string) t.varnames in
...
let of_qualid id =
  let constant_body =
    ...
    | _ -> raise (Invalid_argument "qualid does not indicate constant") in
