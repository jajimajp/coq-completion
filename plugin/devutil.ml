...
let next_binls ls =
  ...
  let rec find_tru_idx i = function
    | [] -> None
    | true :: _ -> Some i
    | false :: rest -> find_tru_idx (i + 1) rest
  in
  ...
  in
  match true_idx with None -> None | Some i -> Some (List.rev (toggle i 0 r))

let insert_unique x l = if List.mem x l then l else x :: l
let merge l1 l2 = List.fold_left (fun acc x -> insert_unique x acc) l1 l2

let string_of_lname lname =
  lname.CAst.v |> Names.Name.print |> Pp.string_of_ppcmds
...
and string_of_local_binder_expr_list ls =
  let inside =
    ls |> List.map string_of_local_binder_expr |> String.concat ","
  in
  "local_binder (" ^ inside ^ ")"
...
and string_of_constr_expr (e : Constrexpr.constr_expr) : string =
  let rec string_of_constrexpr_r = function
    ...
    | CLambdaN (lbel, ce) ->
        "CLambdaN ("
        ^ string_of_local_binder_expr_list lbel
        ^ ", "
        ^ string_of_constrexpr_r ce.CAst.v
        ^ ")"
    | CLetIn (lname, ce1, None, ce2) ->
        "CLetIn (" ^ string_of_lname lname ^ ","
        ^ string_of_constrexpr_r ce1.CAst.v
        ^ ",None,"
        ^ string_of_constrexpr_r ce2.CAst.v
        ^ ")"
  ...
  in
  string_of_constrexpr_r e.CAst.v
