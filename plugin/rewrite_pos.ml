...
let extends_undefined evars evars' =
  let f ev evi found = found || not (Evd.mem evars ev) in
  fold_undefined f evars' false
...
let find_class_proof proof_type proof_method env evars carrier relation =
  try
    let evars, goal =
      app_poly_check env evars proof_type [| carrier; relation |]
    in
    ...
    if extends_undefined (goalevars evars) evars' then raise Not_found
      ...
      app_poly_check env
        (evars', cstrevars evars)
        proof_method [| carrier; relation; c |]
  with e when CErrors.noncritical e -> raise Not_found

let eq_pb ((ty, env, x, y) as pb) ((ty', env', x', y') as pb') =
  let equal x y =
    Constr.equal (EConstr.Unsafe.to_constr x) (EConstr.Unsafe.to_constr y)
  in
...
let evd_convertible env evd x y =
  try
    ...
    let _, pbs' = Evd.extract_all_conv_pbs evd' in
    if evd' == evd || problem_inclusion pbs' pbs then Some evd' else None
...
let error_no_relation () =
  user_err Pp.(str "Cannot find a relation to rewrite.")
...
let rec decompose_app_rel env evd t =
  ...
  | App (f, [| arg |]) ->
      ...
      let f'' =
        mkLambda
          ( make_annot (Name Namegen.default_dependent_ident) r,
            ty,
            mkLambda
              ( make_annot (Name (Id.of_string "y")) r',
                lift 1 ty',
...
let decompose_app_rel env evd t =
  ...
  let ty =
    try Retyping.get_type_of ~lax:true env evd rel
    with Retyping.RetypeError _ -> error_no_relation ()
  in
  if not (Reductionops.is_arity env evd ty) then None
  else
...
let decompose_applied_relation env sigma (c, l) =
              ...
              ( sigma,
                {
                  ...
                  car = ty1;
                  rel = equiv;
                  sort = Sorts.is_prop (ESorts.kind sigma concl);
                  c1;
                  c2;
                  holes;
                } ))
  ...
  | Some c -> c
  | None -> (
      let ctx, t' = Reductionops.whd_decompose_prod env sigma ctype in
      (* Search for underlying eq *)
      let t' =
        it_mkProd_or_LetIn t' (List.map (fun (n, t) -> LocalAssum (n, t)) ctx)
      in
      match find_rel t' with
      | Some c -> c
      | None ->
          user_err Pp.(str "Cannot find an homogeneous relation to rewrite."))
...
struct
  ...
  let relation : env -> evars -> evars * constr = bind_rewrite "relation"
  ...
  let proper_proj () =
    UnsafeMonomorphic.mkConst
      (Option.get (List.hd (proper_class ()).TC.cl_projs).TC.meth_const)
  ...
  let proper_proof env evars carrier relation x =
    let evars, goal =
      app_poly env evars proper_proxy_type [| carrier; relation; x |]
    in
    new_cstr_evar evars env goal

  let get_reflexive_proof env =
    find_class_proof reflexive_type reflexive_proof env

  let get_symmetric_proof env =
    find_class_proof symmetric_type symmetric_proof env

  let get_transitive_proof env =
    find_class_proof transitive_type transitive_proof env

  let mk_relation env evars ty =
    let evars', ty =
      Evarsolve.refresh_universes ~onlyalg:true ~status:(Evd.UnivFlexible false)
        (Some false) env (fst evars) ty
    in
    ...
    let rec aux env evars ty l =
      ...
      | Prod (na, ty, b), obj :: cstrs ->
          ...
          if noccurn (goalevars evars) 1 b (* non-dependent product *) then
            ...
            let evars', b' =
              Evarsolve.refresh_universes ~onlyalg:true
                ~status:(Evd.UnivFlexible false) (Some false) env (fst evars) b'
            in
            let evars, newarg =
              app_poly env
                (evars', snd evars)
                respectful [| ty; b'; relty; arg |]
            in
            ...
            let evars, arg' =
              app_poly env evars forall_relation [| ty; pred; liftarg |]
            in
            if Option.is_empty obj then
              ...
              user_err
                Pp.(
                  str
                    "build_signature: no constraint can apply on a dependent \
                     argument")
      | _, obj :: _ ->
          anomaly ~label:"build_signature" (Pp.str "not enough products.")
      | _, [] -> (
  ...
  let arrow_morphism env evd n ta tb a b =
    ...
    if ap && bp then (app_poly env evd impl [| a; b |], unfold_impl n)
      ...
      (* Domain in Prop, CoDomain in Type *)
      (app_poly env evd arrow [| a; b |], unfold_impl n)
      (* (evd, mkProd (Anonymous, a, b)), (fun x -> x) *)
    else if bp then
      (* Dummy forall *)
      ( app_poly env evd coq_all
          [| a; mkLambda (make_annot n ERelevance.relevant, a, lift 1 b) |],
        unfold_forall )
    else
      (* None in Prop, use arrow *)
      (app_poly env evd arrow [| a; b |], unfold_impl n)
  ...
  let rec decomp_pointwise env sigma n c =
    if Int.equal n 0 then Some c
      ...
      | App (f, [| a; b; relb |])
        when isRefX env sigma (pointwise_relation_ref ()) f ->
          decomp_pointwise env sigma (pred n) relb
      | App (f, [| a; b; arelb |])
        when isRefX env sigma (forall_relation_ref ()) f ->
          decomp_pointwise env sigma (pred n)
            (Reductionops.beta_applist sigma (arelb, [ mkRel 1 ]))
      | _ ->
  ...
  let rec apply_pointwise env sigma rel = function
    | arg :: args -> (
        ...
        | App (f, [| a; b; arelb |])
          when isRefX env sigma (forall_relation_ref ()) f ->
            apply_pointwise env sigma
              (Reductionops.beta_applist sigma (arelb, [ arg ]))
              args
  ...
  let pointwise_or_dep_relation env evars n t car rel =
    ...
    if noccurn (goalevars evars) 1 car && noccurn (goalevars evars) 1 rel then
      app_poly env evars pointwise_relation
        [| t; lift (-1) car; lift (-1) rel |]
    else
      app_poly env evars forall_relation
        [|
          t;
          mkLambda (make_annot n ERelevance.relevant, t, car);
  ...
  let lift_cstr env evars (args : constr list) c ty cstr =
      ...
      if Int.equal n 0 then start evars env prod
        ...
        | Prod (na, ty, b) ->
            if noccurn sigma 1 b then
              ...
              let evars, rb =
                aux evars (push_rel (LocalAssum (na, ty)) env) b (pred n)
              in
              app_poly env evars forall_relation
    ...
    let rec find env c ty = function
      ...
      | arg :: args -> (
          try
            let evars, found = aux evars env ty (succ (List.length args)) in
          ...
          try
            ...
            | Some (equiv, ty1, ty2, concl, c1, c2) -> (
                let (evars, evset), inst =
                  app_poly env' (sigma, Evar.Set.empty) rewrite_relation_class
                    [| ty1; equiv |]
                in
...
module TypeGlobal = struct
  ...
  let inverse env (evd, cstrs) car rel =
    let evd, car =
      Evarsolve.refresh_universes ~onlyalg:true (Some false) env evd car
    in
...
let init_relation_classes () =
  if
    Coqlib.has_ref "rewrite.prop.relation"
    || Coqlib.has_ref "rewrite.type.relation"
  then ()
  else
    CErrors.user_err
      (Pp.str
         "No bindings have been registered for relation classes in Prop or \
          Type, maybe you need to require Coq.Classes.(C)RelationClasses.")

let init_rewrite () =
  if
    Coqlib.has_ref "rewrite.prop.Proper" || Coqlib.has_ref "rewrite.type.Proper"
  then ()
  else
    CErrors.user_err
      (Pp.str
         "No bindings have been registered for morphisms in Prop or Type, \
          maybe you need to require Coq.Classes.(C)Morphisms.")
...
let split_head = function hd :: tl -> (hd, tl) | [] -> assert false
...
let general_rewrite_unif_flags () =
  ...
  in
  {
    ...
    Unification.subterm_unify_flags =
      { core_flags with Unification.modulo_delta = TransparentState.empty };
    Unification.allow_K_in_toplevel_higher_order_unification = true;
    Unification.resolve_evars = true;
  }
  ...
  | Some tac ->
      let map h =
        if h.EClause.hole_deps then None
          ...
          match EConstr.kind sigma h.EClause.hole_evar with
          | Evar (evk, _) -> Some evk
          | _ -> None
      in
      ...
      let solve_tac =
        ...
        | Genarg.GenArg (Genarg.Glbwit tag, tac) ->
            Ftactic.run (Geninterp.interp tag ist tac) (fun _ ->
                Proofview.tclUNIT ())
      in
      let solve_tac = tclCOMPLETE solve_tac in
      let solve sigma evk =
        let evi =
          try Some (Evd.find_undefined sigma evk) with Not_found -> None
        in
        match evi with
        ...
        | Some evi ->
            ...
            let c, sigma =
              Proof.refine_by_tactic ~name ~poly env sigma ty solve_tac
            in
            Evd.define evk c sigma
      in
      List.fold_left solve sigma indep

let no_constraints cstrs ev _ = not (Evar.Set.mem ev cstrs)
...
type 'a strategy_input = {
  ...
  evars : evars;
  poss : int list option (* Rewrite position list *);
}
...
let symmetry env sort rew =
  ...
  let rew_evars, rew_prf =
    match rew.rew_prf with
...
let unify_eqn (car, rel, prf, c1, c2, holes, sort) l2r flags env (sigma, cstrs)
    by t =
  try
    let left = if l2r then c1 else c2 in
    let sigma = Unification.w_unify ~flags env sigma CONV left t in
    let sigma =
      TC.resolve_typeclasses ~filter:(no_constraints cstrs) ~fail:true env sigma
    in
    ...
    match Reductionops.infer_conv ~pb:CUMUL env sigma ty2 ty1 with
    | None -> None
    | Some sigma ->
...
let unify_abs (car, rel, prf, c1, c2) l2r sort env (sigma, cstrs) t =
  try
    ...
    (* The pattern is already instantiated, so the next w_unify is
       ...
       solved this evars *)
    let sigma =
      Unification.w_unify ~flags:rewrite_unif_flags env sigma CONV left t
    in
    let rew_evars = (sigma, cstrs) in
    let rew_prf = RewPrf (rel, prf) in
    let rew =
      { rew_car = car; rew_from = c1; rew_to = c2; rew_prf; rew_evars }
    in
    let rew = if l2r then rew else symmetry env sort rew in
    Some rew
  with e when noncritical e -> None
...
let make_eq env sigma = new_global env sigma Coqlib.(lib_ref "core.eq.type")

let make_eq_refl env sigma =
  new_global env sigma Coqlib.(lib_ref "core.eq.refl")

let get_rew_prf env evars r =
  ...
  | RewCast c ->
      ...
      ( evars,
        ( rel,
          mkCast
            ( mkApp (eq_refl, [| r.rew_car; r.rew_from |]),
              c,
              mkApp (rel, [| r.rew_from; r.rew_to |]) ) ) )
...
let resolve_subrelation env car rel sort prf rel' res =
  if Termops.eq_constr env (fst res.rew_evars) rel rel' then res
    ...
    let evars, app =
      app_poly_check env res.rew_evars (poly_subrelation sort)
        [| car; rel; rel' |]
    in
    let evars, subrel = new_cstr_evar evars env app in
    let appsub = mkApp (subrel, [| res.rew_from; res.rew_to; prf |]) in
    { res with rew_prf = RewPrf (rel', appsub); rew_evars = evars }

let resolve_morphism env m args args' (b, cstr) evars =
  let evars, proj, sigargs, m', args, args' =
    let first =
      match Array.findi (fun _ b -> not (Option.is_empty b)) args' with
      | Some i -> i
      | None -> invalid_arg "resolve_morphism"
    in
    ...
    let evars', appmtype' =
      Evarsolve.refresh_universes ~status:(Evd.UnivFlexible false) ~onlyalg:true
        (Some false) env (fst evars) appmtype'
    in
    ...
    let env' =
      EConstr.push_named
        (LocalDef (make_annot dosub_id ERelevance.relevant, dosub, appsub))
        env
    in
    ...
    Array.fold_left2
      (fun (acc, subst, evars, sigargs, typeargs') x y ->
        ...
        | Some relation -> (
            ...
            | None ->
                let evars, proof =
                  (if b then PropGlobal.proper_proof
                   else TypeGlobal.proper_proof)
                    env evars carrier relation x
                in
                ([ proof; x; x ] @ acc, subst, evars, sigargs, x :: typeargs')
            | Some r ->
                let evars, proof = get_rew_prf env evars r in
                ( [ snd proof; r.rew_to; x ] @ acc,
                  subst,
                  evars,
                  sigargs,
                  r.rew_to :: typeargs' ))
        | None ->
            if not (Option.is_empty y) then
              user_err
                Pp.(
                  str "Cannot rewrite inside dependent arguments of a function");
...
let apply_rule unify : occurrences_count pure_strategy =
  {
    strategy =
      (fun { state = occs; env; term1 = t; ty1 = ty; cstr; evars } ->
        let unif =
          if isEvar (goalevars evars) t then None else unify env evars t
        in
        match unif with
        | None -> (occs, Fail)
        | Some rew ->
            ...
            else if Termops.eq_constr env (fst rew.rew_evars) t rew.rew_to then
              (occs, Identity)
            else
    ...
    strategy =
      (fun ({ state = (); env; term1 = t; evars = sigma, cstrs } as input) ->
        ...
        let unify env evars t =
          ...
          match rew with None -> None | Some rew -> Some rew
        in
        let loccs, res =
          (apply_rule unify).strategy
            { input with state = initialize_occurrence_counter loccs; evars }
        in
...
let make_leibniz_proof env c ty r =
  ...
  in
  {
    rew_car = ty;
    rew_evars = !evars;
    rew_from = subst1 r.rew_from c;
    rew_to = subst1 r.rew_to c;
    rew_prf = prf;
  }
...
let fold_match ?(force = false) env sigma c =
  ...
  let dep, pred, sk =
    ...
    let pred =
      if dep then p else it_mkProd_or_LetIn (subst1 mkProp body) (List.tl ctx)
    in
    let sk =
      (* not sure how correct this is *)
      if sortp == Sorts.InProp then
        if sortc == Sorts.InProp then if dep then case_dep else case_nodep
        else if dep then casep_dep
        else case_nodep (* should this be casep_nodep? *)
      else if (* sortc <> InProp by typing *)
              dep then case_dep
      else case_nodep
    in
  ...
  let app =
    let sk =
      if Global.is_polymorphic (ConstRef sk) then
        CErrors.anomaly Pp.(str "Unexpected univ poly in Rewrite.fold_match")
...
let unfold_match env sigma sk app =
  ...
  | App (f', args) when QConstant.equal env (fst (destConst sigma f')) sk ->
      let v =
        Environ.constant_value_in env (sk, UVars.Instance.empty)
        (*FIXME*)
      in
...
let subterm all flags (s : 'a pure_strategy) : 'a pure_strategy =
  let rec aux
    ...
    | App (m, args) ->
              ...
              (fun (i, state, (acc, evars, progress)) arg ->
                if (not (Option.is_empty progress)) && not all then
                  ...
                  let cont, poss =
                    ...
                    if isRefX env sigma (coq_all_ref ()) m then (false, poss)
                    else
                      match poss with
                      | Some [] -> (true, poss)
                  ...
                  if cont then
                    ...
                    let res' =
                      ...
                      | Identity ->
                          let progress =
                            if Option.is_empty progress then Some false
                            else progress
                          in
                          (None :: acc, evars, progress)
                      | Success r -> (Some r :: acc, r.rew_evars, Some true)
                      | Fail -> (None :: acc, evars, progress)
                    in
                    (succ i, state, res'))
              (0, state, ([], evars, success))
              args
          in
          let res =
            match progress with
            ...
            | Some true ->
                ...
                if
                  Array.exists
                    (function
                      | None -> false | Some r -> not (is_rew_cast r.rew_prf))
                    args'
                then
                  ...
                  let args' =
                    Array.map2
                      (fun aorig anew ->
                        match anew with None -> aorig | Some r -> r.rew_to)
                      args args'
                  in
                  let res =
                    {
        ...
        if flags.on_morphisms then
          ...
          let evars, cstr', m, mty, argsl, args =
            ...
            let lift =
              if prop then PropGlobal.lift_cstr else TypeGlobal.lift_cstr
            in
          ...
          | Fail ->
              rewrite_args state
                None (* Standard path, try rewrite on arguments *)
          ...
          | Success r ->
              ...
              let prf =
                ...
                | RewPrf (rel, prf) ->
                    let app =
                      if prop then PropGlobal.apply_pointwise
                      else TypeGlobal.apply_pointwise
                    in
                    RewPrf
                      (app env (goalevars evars) rel argsl, mkApp (prf, args))
                | x -> x
              in
              let res =
                {
                  rew_car =
                    Reductionops.hnf_prod_appvect env (goalevars evars)
                      r.rew_car args;
                  rew_from = mkApp (r.rew_from, args);
                  rew_to = mkApp (r.rew_to, args);
                  rew_prf = prf;
              ...
              let res =
                ...
                | RewPrf (rel, prf) ->
                    Success
                      (apply_constraint env res.rew_car rel prf (prop, cstr) res)
                | _ -> Success res
              in
              (state, res)
    ...
    | Prod (n, x, b) when noccurn (goalevars evars) 1 b ->
        ...
        let arr =
          if prop then PropGlobal.arrow_morphism else TypeGlobal.arrow_morphism
        in
        ...
        let res =
          ...
          | Success r ->
              Success
                { r with rew_to = unfold (goalevars r.rew_evars) r.rew_to }
          | Fail | Identity -> res
        in
        (state, res)
    | Prod (n, dom, codom) ->
        ...
        let (evars', app), unfold =
          if eq_constr (fst evars) ty mkProp then
            ( app_poly_sort prop env evars coq_all [| dom; lam |],
              TypeGlobal.unfold_all )
          else
            let forall =
              if prop then PropGlobal.coq_forall else TypeGlobal.coq_forall
            in
            ( app_poly_sort prop env evars forall [| dom; lam |],
              TypeGlobal.unfold_forall )
        in
        (* let state, res = aux { state ; env ; unfresh ;
                               term1 = app ; ty1 = ty ;
                               cstr = (prop,cstr) ; evars = evars'; poss } in *)
    ...
    *)
    | Lambda (n, t, b) when flags.under_lambdas ->
        ...
        let unlift =
          if prop then PropGlobal.unlift_cstr else TypeGlobal.unlift_cstr
        in
        let state, b' =
          s.strategy
            {
        ...
        let res =
          ...
          | Success r ->
              let r =
                ...
                | RewPrf (rel, prf) ->
                    let point =
                      if prop then PropGlobal.pointwise_or_dep_relation
                      else TypeGlobal.pointwise_or_dep_relation
                    in
                    let evars, rel =
                      point env r.rew_evars n'.binder_name t r.rew_car rel
                    in
    ...
    | Case (ci, u, pms, p, iv, c, brs) ->
        let ci, (p, rp), iv, c, brs =
          EConstr.expand_case env (goalevars evars) (ci, u, pms, p, iv, c, brs)
        in
        ...
        let state, res =
          ...
          | Success r ->
              let case =
                mkCase
                  (EConstr.contract_case env (goalevars evars)
                     ( ci,
                       (lift 1 p, rp),
                       map_invert (lift 1) iv,
                       mkRel 1,
                       Array.map (lift 1) brs ))
              in
          ...
          | Fail | Identity -> (
              if Array.for_all (Int.equal 0) ci.ci_cstr_ndecls then
                let evars', eqty =
                  app_poly_sort prop env evars coq_eq [| ty |]
                in
                let cstr = Some eqty in
                let state, found, brs' =
                  Array.fold_left
                    (fun (state, found, acc) br ->
                      if not (Option.is_empty found) then
                        ...
                        | Fail | Identity ->
                            (state, None, fun x -> lift 1 br :: acc x))
                    (state, None, fun x -> [])
                    brs
                in
                match found with
                      ...
                      mkCase
                        (EConstr.contract_case env (goalevars evars)
                           ( ci,
                             ...
                             map_invert (lift 1) iv,
                             lift 1 c,
                             Array.of_list (List.rev (brs' c')) ))
                    in
                    (state, Success (make_leibniz_proof env ctxc ty r))
                | None -> (state, c')
              else
                match
                  try Some (fold_match env (goalevars evars) t)
                  with Not_found -> None
                with
                | None -> (state, c')
                | Some (cst, t') ->
                          ...
                          Success
                            {
                              ...
                              rew_from = t;
                              rew_to =
                                unfold_match env (goalevars evars) cst
                                  prf.rew_to;
                            }
                      | x' -> c'
                    in
...
let transitivity state env unfresh cstr (res : rewrite_result_info)
    (next : 'a pure_strategy) : 'a * rewrite_result =
  let cstr =
    match cstr with
    | _, Some _ -> cstr
    ...
    | Success res' -> (
        ...
        | RewPrf (rew_rel, rew_prf) -> (
            ...
            | RewPrf (res'_rel, res'_prf) ->
                ...
                in
                let evars, prfty =
                  app_poly_sort (fst cstr) env res'.rew_evars trans
                    [| res.rew_car; rew_rel |]
                in
...
module Strategies = struct
  let fail : 'a pure_strategy = { strategy = (fun { state } -> (state, Fail)) }
  ...
  let refl : 'a pure_strategy =
    {
      strategy =
        (fun { state; env; term1 = t; ty1 = ty; cstr = prop, cstr; evars } ->
          let evars, rel =
            match cstr with
            | None ->
                let mkr =
                  if prop then PropGlobal.mk_relation
                  else TypeGlobal.mk_relation
                in
          ...
          let evars, proof =
            ...
            in
            let evars, mty =
              app_poly_sort prop env evars proxy [| ty; rel; t |]
            in
  ...
  let fix (f : 'a pure_strategy -> 'a pure_strategy) : 'a pure_strategy =
    let rec aux input =
      (f { strategy = (fun input -> check_interrupt aux input) }).strategy input
    in
    { strategy = aux }
  ...
  let repeat (s : 'a pure_strategy) : 'a pure_strategy = seq s (any s)
  ...
  let inj_open hint =
    ...
    fun sigma ->
      ...
      let lemma =
        Vars.subst_univs_level_constr subst (EConstr.of_constr lemma)
      in
  ...
  let reduce (r : Redexpr.red_expr) : 'a pure_strategy =
    {
      strategy =
        (fun { state; env; term1 = t; ty1 = ty; cstr; evars } ->
          ...
          if Termops.eq_constr env sigma t' t then (state, Identity)
          else
            ( state,
              Success
                {
  ...
  let fold_glob c : 'a pure_strategy =
    {
      strategy =
        (fun { state; env; term1 = t; ty1 = ty; cstr; evars } ->
          ...
          let unfolded =
            match Tacred.red_product env sigma c with
            | None -> user_err Pp.(str "fold: the term is not unfoldable!")
            | Some c -> c
          in
          try
            let sigma =
              Unification.w_unify env sigma CONV
      ...
      (fun ({ state = () } as input) ->
        ...
        let strat =
          Strategies.fix (fun aux ->
              Strategies.choice (Strategies.progress app)
                (subterm true default_flags aux))
        in
        let occs, res =
          strat.strategy
            { input with state = initialize_occurrence_counter occs }
        in
...
let solve_constraints env (evars, cstrs) =
  ...
  let evars' =
    TC.resolve_typeclasses env ~filter:TC.all_evars ~fail:true evars'
  in
  Evd.set_typeclass_evars evars' oldtcs

let nf_zeta = Reductionops.clos_norm_flags (RedFlags.mkflags [ RedFlags.fZETA ])
...
let () =
  CErrors.register_handler (function
    | UnsolvedConstraints (env, evars, ev) ->
        Some
          (str "Unsolved constraint remaining: "
          ++ spc ()
          ++ Termops.pr_evar_info env evars (Evd.find_undefined evars ev)
          ++ str ".")
    | _ -> None)

let cl_rewrite_clause_aux ?(abs = None) strat env avoid sigma concl is_hyp :
    result =
  ...
  let evars, cstr =
    let prop, (evars, arrow) =
      if ESorts.is_prop sigma sort then
        (true, app_poly_sort true env evars impl [||])
  ...
  | Success res ->
      ...
      let iter ev =
        if not (Evd.is_defined evars ev) then
          raise (UnsolvedConstraints (env, evars, ev))
      in
...
let cl_rewrite_clause_newtac ?abs ?origsigma ~progress strat clause =
  ...
  (* For compatibility *)
  let beta =
    Tactics.reduct_in_concl ~cast:false ~check:false
      (Reductionops.nf_betaiota, DEFAULTcast)
  in
  let beta_hyp id =
    Tactics.reduct_in_hyp ~check:false ~reorder:false Reductionops.nf_betaiota
      (id, InHyp)
  in
  let treat sigma res state =
    ...
    | Some None ->
        if progress then newfail 0 (str "Failed to progress")
    ...
    | Some (Some res) -> (
        ...
        | Some id, None ->
            Proofview.Unsafe.tclEVARS undef
            <*> convert_hyp ~check:false ~reorder:false
                  (LocalAssum (make_annot id ERelevance.relevant, newt))
            <*> beta_hyp id
        | None, Some p ->
            Proofview.Unsafe.tclEVARS undef
            <*> Proofview.Goal.enter (fun gl ->
                    ...
                    let make sigma =
                      ...
                      (sigma, mkApp (p, [| ev |]))
                    in
                    Refine.refine ~typecheck:true make
                    <*> Proofview.Unsafe.tclNEWGOALS gls)
        | None, None ->
            Proofview.Unsafe.tclEVARS undef
  ...
  Proofview.Goal.enter (fun gl ->
      ...
      try
        ...
        in
        let sigma =
          match origsigma with None -> sigma | Some sigma -> sigma
        in
        treat sigma res state
        <*> (* For compatibility *)
        beta <*> Proofview.shelve_unifiable
      with PretypeError (env, evd, (UnsatisfiableConstraints _ as e)) ->
...
let cl_rewrite_clause_strat progress strat clause =
  ...
  *)
  ...
  <*> (if progress then Proofview.tclPROGRESS else fun x -> x)
        (Proofview.tclOR (cl_rewrite_clause_newtac ~progress strat clause)
           (fun (e, info) ->
             match e with
             | Tacticals.FailError (n, pp) ->
                 tclFAILn ~info n
                   (str "setoid rewrite failed: " ++ Lazy.force pp)
...
(** strategy for rewrite_pos *)
let rewrite_with_pos l2r flags c occs poss : strategy =
  {
    strategy =
      (fun ({ state = () } as input) ->
        (* Update input.poss *)
        let input = { input with poss = Some poss } in
...
let rewrite_pos_strat strat clause =
  tactic_init_rewrite ()
  <*> Proofview.tclPROGRESS
        (Proofview.tclOR (cl_rewrite_clause_newtac ~progress:true strat clause)
           (fun (e, info) ->
             match e with
             | Tacticals.FailError (n, pp) ->
                 tclFAILn ~info n (str "rewrite_pos failed: " ++ Lazy.force pp)
             | e -> Proofview.tclZERO ~info e))

let rewrite_pos l l2r occs clause poss =
  let strat =
    rewrite_with_pos l2r (general_rewrite_unif_flags ()) l occs poss
  in
  rewrite_pos_strat strat clause

let apply_glob_constr ((_, c) : _ * EConstr.t delayed_open) l2r occs =
  ();
...
let rec map_strategy f g h = function
  ...
  | StratBinary (s, str, str') ->
      StratBinary (s, map_strategy f g h str, map_strategy f g h str')
...
and pr_strategy1 prc prr prid = function
  ...
  | StratNAry (Choice, strs) ->
      ...
      ++ brk (1, 2)
      ++ prlist_with_sep spc
           (fun str -> hov 0 (pr_strategy0 prc prr prid str))
           strs
...
and pr_strategy2 prc prr prid = function
  | StratBinary (Compose, str1, str2) ->
      pr_strategy2 prc prr prid str1
      ++ str ";" ++ spc ()
      ++ hov 0 (pr_strategy1 prc prr prid str2)
  | str -> hov 0 (pr_strategy1 prc prr prid str)

and pr_strategy prc prr prid = function
  | StratFix (id, s) ->
      str "fix" ++ spc () ++ prid id ++ spc () ++ str ":=" ++ spc ()
      ++ hov 0 (pr_strategy1 prc prr prid s)
...
let rec strategy_of_ast bindings = function
  ...
  | StratBinary (f, s, t) ->
      ...
      let f' = match f with Compose -> Strategies.seq in
      f' s' t'
  | StratNAry (Choice, strs) -> (
  ...
  | StratHints (old, id) ->
      if old then Strategies.old_hints id else Strategies.hints id
  | StratTerms l ->
      {
        strategy =
          (fun ({ state = (); env } as input) ->
  ...
  | StratEval r ->
      {
        strategy =
          (fun ({ state = (); env; evars } as input) ->
            ...
            (Strategies.reduce r_interp).strategy
              { input with evars = (sigma, cstrevars evars) });
      }
  | StratFold c -> Strategies.fold_glob (fst c)
  | StratVar id -> Id.Map.get id bindings
  | StratFix (id, s) ->
      Strategies.fix (fun self ->
          strategy_of_ast (Id.Map.add id self bindings) s)
...
let proper_projection sigma r ty =
  ...
  let app =
    mkApp (PropGlobal.proper_proj (), Array.append args [| instarg |])
  in
...
let build_morphism_signature env sigma m =
  ...
  let cstrs =
    let rec aux t =
      match EConstr.kind sigma t with
      | Prod (na, a, b) -> None :: aux b
      | _ -> []
    in
    aux t
  in
  let evars, t', sig_, cstrs =
    PropGlobal.build_signature (sigma, Evar.Set.empty) env t cstrs None
  in
...
let default_morphism env sigma sign m =
  ...
  let evars, _, sign, cstrs =
    PropGlobal.build_signature (sigma, Evar.Set.empty) env t (fst sign)
      (snd sign)
  in
  let evars, morph =
    app_poly_check env evars PropGlobal.proper_type [| t; sign; m |]
  in
  ...
  let sigma, c' =
    try
      (* ~flags:(false,true) to allow to mark occurrences that must not be
         ...
         in the context *)
      Unification.w_unify_to_subterm ~flags:rewrite_unif_flags env sigma
        ((if l2r then c1 else c2), but)
    with ex when Pretype_errors.precatchable_exception ex ->
      (* ~flags:(true,true) to make Ring work (since it really
         exploits conversion) *)
      Unification.w_unify_to_subterm ~flags:rewrite_conv_unif_flags env sigma
        ((if l2r then c1 else c2), but)
  in
  let nf c = Reductionops.nf_evar sigma c in
...
let general_s_rewrite cl l2r occs (c, l) ~new_goals =
  Proofview.Goal.enter (fun gl ->
      ...
      let app = apply_rule unify in
      let recstrat aux =
        Strategies.choice app (subterm true general_rewrite_flags aux)
      in
            ...
            (tclPROGRESS
               (tclTHEN
                  ...
                  (cl_rewrite_clause_newtac ~progress:true ~abs:(Some abs)
                     ~origsigma strat cl)))
            (fun (e, info) -> match e with e -> Proofview.tclZERO ~info e))
...
exception
  RelationNotDeclared of Environ.env * Evd.evar_map * string * EConstr.types
...
let () =
  CErrors.register_handler (function
    | RelationNotDeclared (env, sigma, ty, concl) ->
        ...
        Some
          (str " The relation "
          ++ Printer.pr_econstr_env env sigma rel
          ++ str " is not a declared " ++ str ty
          ++ str
               " relation. Maybe you need to require the \
...
let setoid_proof ty fn fallback =
  Proofview.Goal.enter (fun gl ->
      ...
      Proofview.tclORELSE
        (try
           let rel, ty1, ty2, concl, _, _ =
             decompose_app_rel_error env sigma concl
           in
           let sigma, t = Typing.type_of env sigma rel in
           let car =
             snd (List.hd (fst (Reductionops.whd_decompose_prod env sigma t)))
           in
           (try init_relation_classes ()
            with e when CErrors.noncritical e -> raise Not_found);
        ...
        (function
          | e ->
              Proofview.tclORELSE fallback (function e', info ->
                  (match e' with
...
let tac_open ((evm, _), c) tac = tclTHEN (Proofview.Unsafe.tclEVARS evm) (tac c)
...
let setoid_reflexivity =
  setoid_proof "reflexive"
    (fun env evm car rel ->
      tac_open
        (poly_proof PropGlobal.get_reflexive_proof
           TypeGlobal.get_reflexive_proof env evm car rel) (fun c ->
          tclCOMPLETE (apply c)))
...
let setoid_symmetry =
  setoid_proof "symmetric"
    (fun env evm car rel ->
      tac_open
        (poly_proof PropGlobal.get_symmetric_proof
           TypeGlobal.get_symmetric_proof env evm car rel) (fun c -> apply c))
    (symmetry_red true)
...
let setoid_transitivity c =
  setoid_proof "transitive"
    (fun env evm car rel ->
      tac_open
        (poly_proof PropGlobal.get_transitive_proof
           TypeGlobal.get_transitive_proof env evm car rel) (fun proof ->
          match c with
          | None -> eapply proof
...
let setoid_symmetry_in id =
  Proofview.Goal.enter (fun gl ->
      ...
      let rec split_last_two = function
        | [ c1; c2 ] -> ([], (c1, c2))
        | x :: y :: z ->
            let l, res = split_last_two (y :: z) in
            (x :: l, res)
        | _ ->
            user_err Pp.(str "Cannot find an equivalence relation to rewrite.")
      in
      ...
      tclTHENLAST
        ...
        (tclTHENLIST
           [ intros; setoid_symmetry; apply (mkVar id); Tactics.assumption ]))
...
let get_reflexive_proof = get_lemma_proof PropGlobal.get_reflexive_proof
let get_symmetric_proof = get_lemma_proof PropGlobal.get_symmetric_proof
let get_transitive_proof = get_lemma_proof PropGlobal.get_transitive_proof

module Internal = struct
  let build_signature env sigma m cstr finalcstr =
    let evars = (sigma, Evar.Set.empty) in
    let (sigma, _), _, sig_, cstr =
      PropGlobal.build_signature evars env m cstr finalcstr
    in
    (sigma, sig_, cstr)
