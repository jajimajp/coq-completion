...
let subterm all flags (s : 'a pure_strategy) : 'a pure_strategy =
  let rec aux { state ; env ; unfresh ;
                term1 = t ; ty1 = ty ; cstr = (prop, cstr) ; evars; poss } =
...
let apply_strategy (s : strategy) env unfresh concl (prop, cstr) evars =
  ...
  let _, res = s.strategy { state = () ; env ; unfresh ;
                            term1 = concl ; ty1 = ty ;
                            cstr = (prop, Some cstr) ; evars; poss=None } in
  res
...
let rewrite_pos_strat strat clause =
  tactic_init_rewrite () <*>
    ...
    (Proofview.tclOR
      (cl_rewrite_clause_newtac ~progress:true strat clause)
      (fun (e, info) -> match e with
       | Tacticals.FailError (n, pp) ->
...
let rewrite_pos l l2r occs clause poss =
  let strat = rewrite_with_pos l2r (general_rewrite_unif_flags ()) l occs poss in
