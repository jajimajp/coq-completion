...
let string_of_chars (l : char list) = l |> List.to_seq |> String.of_seq
...
let rec expect str lines : string list =
  ...
  | hd :: tl ->
      if hd = str then tl
      else if hd = "" then expect str tl
      else failwith ("expect: Unexpected output " ^ hd ^ ", expected " ^ str)
...
let expect_order lines : string list * string list =
  ...
  | hd :: tl ->
      ...
      if Str.string_match re hd 0 then
        ...
        (order, tl)
      else failwith ("expect_order: Unexpected output " ^ hd)
...
let expect_axioms lines : string list * string list =
  let rec aux lines acc =
    match lines with
    | line :: rest ->
        if line = "generated rules:" then (List.rev acc, lines)
        else aux rest (line :: acc)
    | [] -> failwith "expect_axioms: reached end of input"
  in
  aux lines []

(* {example} - rhs by equation 2 L->R at [0,1] *)
let parse_rewstep : string -> rewstep option =
 fun line ->
  let err = failwith in
  let re =
    Str.regexp
      "- \\(.hs\\) by equation \\([0-9]+\\) \\(....\\) at \\[\\(.*\\)\\]$"
  in
  if Str.string_match re line 0 then
    ...
    let pos =
      if pos = "" then []
      else String.split_on_char ',' pos |> List.map int_of_string
    in
...
let rec consume_proof lines : minimal_strat proof option * string list =
  ...
  | line :: rest -> (
      if line = "" then consume_proof rest
      else if line = "ES:" then (None, lines)
      else
        let parse_header line : rule * bool =
          (* (rule, is_reversed(parsed "<-")) *)
          ...
          if Str.string_match re line 0 then
            ...
            | "<-" -> ((id, (r, l)), true)
            | _ -> ((id, (l, r)), false)
          else failwith "parse_header: invalid toma rule"
        in
        ...
        let parse_crit line =
          ...
          let re =
            Str.regexp
              "^Proof: A critical pair between equations \\([0-9]+\\) and \
               \\([0-9]+\\) with superposition \\(.+\\)\\.$"
          in
          if Str.string_match re line 0 then
            ...
            MCrit (id1, id2, term)
          else failwith "parse_crit: invalid toma rule"
        in
        let parse_simp lines : minimal_strat * string list =
          ...
          | l1 :: rest ->
              ...
              let id =
                if Str.string_match re1 l1 0 then Str.matched_group 1 l1
                else err l1
              in
              let rec read_steps :
                  rewstep list -> string list -> rewstep list * string list =
               fun acc -> function
                | [] -> (List.rev acc, [])
                | hd :: tl -> (
                    match parse_rewstep hd with
                    | Some step -> read_steps (step :: acc) tl
        ...
        | "Proof:" :: "A" :: "critical" :: _ ->
            (Some (rule, parse_crit (List.hd rest)), List.tl rest)
        | "Proof:" :: "Rewrite" :: _ ->
...
let expect_completed_rules lines : rule list =
  let parse_rule line : rule =
    ...
    if Str.string_match re line 0 then
      ...
      let l = termast_of_string l in
      let r = termast_of_string r in
      match op with "<-" -> (id, (r, l)) | _ -> (id, (l, r))
    else failwith "parse_rule: invalid toma rule"
  in
  let rec aux lines acc =
    match lines with
    | [] -> List.rev acc
    | hd :: tl -> if hd = "" then aux tl acc else aux tl (parse_rule hd :: acc)
  in
  aux lines []
...
let parse_minimal lines =
  ...
  let rec aux lines acc : minimal_strat proof list * string list =
    ...
    | hd :: tl -> (
        ...
        | Some (rule, strat), rest -> aux rest ((rule, strat) :: acc))
  in
...
let parse lines =
  ...
  let rec aux (proofs : minimal_strat proof list) acc : strat proof list =
    ...
    | (rule, strat) :: tl -> (
        ...
        | MCrit (id1, id2, sp) ->
            aux tl ((rule, Crit (find id1, find id2, sp)) :: acc)
...
let rec consume_goal_simp lines : goal_strat option =
  ...
  let parse_header line : (rule * bool) option =
    (* (rule, is_reversed(parsed "<-")) *)
    (* (ex) 10: +(c1(), 0()) = c1(). *)
    let re =
      Str.regexp "^\\([0-9]+\\): \\(.+\\) \\(->\\|=\\|<-\\) \\(.+\\)\\.$"
  ...
  | l0 :: l1 :: rest -> (
      if l0 = "" then consume_goal_simp (List.tl lines)
        ...
        | None -> None
        | Some (rule, _) ->
            let re1 = Str.regexp "^Proof: Rewrite goal,$" in
...
let add_prefix procedure prefix =
  ...
  let for_rule (id, (l, r)) = (prefix ^ id, (l, r)) in
  let for_strat = function
    ...
    | Simp (r, steps) ->
        Simp
          ( for_rule r,
            List.map (fun step -> { step with rule = prefix ^ step.rule }) steps
          )
  in
